...
system.time(pop2022 <- fread("./Data/buoyData-jan-2022.csv"))
system.time(pop2023 <- fread("./Data/buoyData-jan-2023.csv"))
...
sample1 = na.omit(pop2022) %>%
  ...
  sample_n(get_sample_size(subset(pop2022, station == buoy), confidence.level, confidence.interval))

sample2 = na.omit(pop2023) %>%
  filter(station == buoy) %>%
  select(sea_temperature, station) %>%
  mutate(sample_id = '2023') %>%
  sample_n(get_sample_size(subset(pop2023, station == buoy), confidence.level, confidence.interval))
...
toPlot = summarise(group_by(samples, sample_id), mean = mean(sea_temperature))
...
ggplot(samples) +
  aes(x = sample_id, y = sea_temperature) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()

hist(subset(samples, sample_id == "2022")$sea_temperature,
     main = "M4 Buoy Jan/2022 temperature",
     xlab = "Celsius",
     breaks = 10
)

hist(subset(samples, sample_id == "2023")$sea_temperature,
     main = "M4 Buoy Jan/2023 temperature",
     xlab = "Celsius",
     breaks = 10
)
...
##### Normalitiy tests
res_aov <- aov(sea_temperature ~ sample_id, data = samples)
...
wilcox.test(samples$sea_temperature ~ samples$sample_id)
...
ggbetweenstats(
  ...
  title = "M4 Buoy Sea Temperature Comparison",
  ylab = "Celsius",
  xlab = "Year"
)
