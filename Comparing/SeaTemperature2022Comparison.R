...
pop2022 %>% group_by(station) %>%
  summarise(
    count = n(),
    mean = mean(sea_temperature, na.rm = TRUE),
    sd = sd(sea_temperature, na.rm = TRUE),
    var = var(sea_temperature, na.rm = TRUE),
    cov = (sd(sea_temperature, na.rm = TRUE) / mean(sea_temperature, na.rm = TRUE)),
    median = median(sea_temperature, na.rm = TRUE),
    min = min(sea_temperature, na.rm = TRUE),
    max = max(sea_temperature, na.rm = TRUE),
    range = (max(sea_temperature, na.rm = TRUE) - min(sea_temperature, na.rm = TRUE)),
    p95 = quantile(sea_temperature, 0.95, na.rm = TRUE),
    p99 = quantile(sea_temperature, 0.99, na.rm = TRUE)
  )

ggplot(pop2022) +
  aes(x = station, y = sea_temperature) +
...
confidence.level <- cl99
confidence.interval <- ci99
...
sample1 = na.omit(pop2022) %>%
  filter(station == 'M2') %>%
  select(sea_temperature, station) %>%
...
se1 = sd(sample1$sea_temperature) / sqrt(nrow(sample1))
lower = mean(sample1$sea_temperature) - confidence.level * se1
upper = mean(sample1$sea_temperature) + confidence.level * se1
ci1 = c(lower, upper)
my_data[1, ] <- c('M2', round(mean(sample1$sea_temperature), 2), round(ci1[1], 2), round(ci1[2], 2)) 

## Confidence Interval - Sample 2
se2 = sd(sample2$sea_temperature) / sqrt(nrow(sample2))
lower = mean(sample2$sea_temperature) - confidence.level * se2
upper = mean(sample2$sea_temperature) + confidence.level * se2
ci2 = c(lower, upper)
my_data[2, ] <- c('M3', round(mean(sample2$sea_temperature), 2), round(ci2[1], 2), round(ci2[2], 2)) 

## Confidence Interval - Sample 3
se3 = sd(sample3$sea_temperature) / sqrt(nrow(sample3))
lower = mean(sample3$sea_temperature) - confidence.level * se3
upper = mean(sample3$sea_temperature) + confidence.level * se3
ci3 = c(lower, upper)
my_data[3, ] <- c('M4', round(mean(sample3$sea_temperature), 2), round(ci3[1], 2), round(ci3[2], 2)) 

## Confidence Interval - Sample 4
se4 = sd(sample4$sea_temperature) / sqrt(nrow(sample4))
lower = mean(sample4$sea_temperature) - confidence.level * se4
upper = mean(sample4$sea_temperature) + confidence.level * se4
ci4 = c(lower, upper)
my_data[4, ] <- c('M5', round(mean(sample4$sea_temperature), 2), round(ci4[1], 2), round(ci4[2], 2)) 

## Confidence Interval - Sample 5
se5 = sd(sample5$sea_temperature) / sqrt(nrow(sample5))
lower = mean(sample5$sea_temperature) - confidence.level * se5
upper = mean(sample5$sea_temperature) + confidence.level * se5
ci5 = c(lower, upper)
my_data[5, ] <- c('M6', round(mean(sample5$sea_temperature), 2), round(ci5[1], 2), round(ci5[2], 2)) 
...
hist(subset(samples, station == "M2")$sea_temperature,
     main = "M2 Sea temperature",
     xlab = "Celsius",
     breaks = 10
)

hist(subset(samples, station == "M3")$sea_temperature,
     main = "M3 Sea temperature",
     xlab = "Celsius",
     breaks = 10
)

hist(subset(samples, station == "M4")$sea_temperature,
     main = "M4 Sea temperature",
     xlab = "Celsius",
     breaks = 10
)

hist(subset(samples, station == "M5")$sea_temperature,
     main = "M5 Sea temperature",
     xlab = "Celsius",
     breaks = 10
)

hist(subset(samples, station == "M6")$sea_temperature,
     main = "M6 Sea temperature",
     xlab = "Celsius",
     breaks = 10
)
...
##### Normalitiy tests
res_aov <- aov(sea_temperature ~ station, data = samples)
...
shapiro.test(sample1$sea_temperature) # p-value = 0.0000001036
shapiro.test(sample2$sea_temperature) # p-value = 0.00000001722
...
ad.test(sample1$sea_temperature) # p-value = 0.0000007717
ad.test(sample2$sea_temperature) # p-value = 0.0000000004449
...
ks.test(sample1$sea_temperature, sample2$sea_temperature) # p-value = 0.03315
...
kruskal.test(sea_temperature ~ station,
             data = samples
)

dunnTest(sea_temperature ~ station,
         data = samples,
         method = "holm")
...
ggbetweenstats(
  ...
  title = "Sea Temperature Comparison",
  ylab = "Celsius",
  xlab = "Station"
)

library(coin)
conover_test(sea_temperature ~ as.factor(station), data = samples)

#samples$station <- as.factor(samples$station)

pairwise.wilcox.test(samples$sea_temperature, samples$station, p.adjust.method = "bonferroni")
